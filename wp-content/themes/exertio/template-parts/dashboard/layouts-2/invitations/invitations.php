...
$query = "SELECT * FROM ".$table_name ." WHERE `freelancer_id` = " . $freelancer_id;
      ...
      <div class="d-flex justify-content-between flex-wrap">
        <div class="d-flex align-items-end flex-wrap">
          <div class="mr-md-3 mr-xl-5">
            <h2><?php echo esc_html__('Job Invitations','exertio_theme'); ?>
              ( <?php echo $total ?> ) 
            </h2>
      ...
      <div class="card mb-4">
        <div class="card-body">
          <div class="pro-section">
            <div class="pro-box heading-row">
              ...
              <div class="pro-coulmn"><?php echo esc_html__( 'Cost / Type', 'exertio_theme' ) ?> </div>
               <div class="pro-coulmn"><?php echo esc_html__( 'Employer', 'exertio_theme' ) ?> </div>
            ...
            if ( !empty($results) )
			{
              ...
              foreach($results as $result)
			  {
               ...
               $price = get_post_meta($pid, '_project_cost', true); 
							 if($type == 'fixed' || $type == 1)
							{
							 	...
							 	echo esc_html(fl_price_separator(  $price)).'/'.esc_html__( 'Fixed ', 'exertio_theme' );
							// 	echo '<small class="estimated-hours">'.esc_html__( 'Days to Complete ', 'exertio_theme' ).$result->day_to_complete.'</small>';
							 ...
							 {
							 	echo esc_html(fl_price_separator( $price )).' '.esc_html__( 'Hourly ', 'exertio_theme' );
             ...
             <?php 
                ...
                $emp_id = get_user_meta($result->employer_id, 'employer_id', true);
                  ...
                  echo esc_html( $emp_name, 'exertio_theme' );
         ...
         <div class="pro-coulmn">
            ...
            <?php if($result->status != "2" && $result->status != "4" ){ ?>
						  <a href="javascript:void(0)" class="btn btn-inverse-primary btn-sm change-invite-status btn-loading" data-status ="accept" data-fid = "<?php echo esc_attr($freelancer_id ); ?>" data-pid="<?php echo esc_attr($result->project_id ); ?>"  data-offer-id ="<?php  echo $result->id ?>" > <i class="far fa-edit"></i> <?php echo esc_html__( 'Accept', 'exertio_theme' ); ?><span class="bubbles"> <i class="fa fa-circle" aria-hidden="true"></i> <i class="fa fa-circle" aria-hidden="true"></i> <i class="fa fa-circle" aria-hidden="true"></i> </span></a>
            <?php  } ?>
	
              <?php if($result->status != "3" && $result->status != "4" ){ ?>
						  <a href="javascript:void(0)" class="btn btn-inverse-danger btn-sm change-invite-status" data-status ="reject" data-fid = "<?php echo esc_attr($freelancer_id ); ?>" data-pid="<?php echo esc_attr($result->project_id ); ?>"  data-offer-id ="<?php echo $result->id ?>"> <i class="far fa-times-circle"></i> <?php echo esc_html__( 'Reject', 'exertio_theme' ); ?></a>
                     <?php  } ?>
					  </span> 
				  </div>
				</div>
